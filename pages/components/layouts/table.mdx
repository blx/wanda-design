...
This component extends the `react-data-table-component` library, which expones different props that enables different functionalities including(but not limited to) `fixedHeader`, `pagination`, `striped`, `responsive` and `selectableRows`. [For a complete reference here's the maintainer's storybook](https://react-data-table-component.netlify.app/?path=/docs/getting-started-kitchen-sink-ts--kitchen-sink-ts). Down here are listed the props that Wanda adds to the default library component.
  ...
  properties={[
    {
      ...
      dangerous: false,
      description:
        "Set the component to use render when an expandable row is open",
    },
    {
