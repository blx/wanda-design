...
Add a vibrancy effect to any element:

<Tab.Root>
  <Tab.Panel icon="layer-group" label="List">

```css hideCopy
@define-mixin vibrancy {
  backdrop-filter: saturate(180%) blur(62px);
...
@define-mixin vibrancy-hover {
  background: linear-gradient(
    to bottom,
    var(--global-vibrancy-background-hover),
    var(--global-vibrancy-background-hover)
  );
}

@define-mixin vibrancy-disabled {
...
```
  ...
  <Tab.Panel icon="arrow-right" label="Usage">

```css hideCopy
@import "@wonderflow/react-components/core/vibrancy.css";
