...
<Snackbar kind="info">
  ...
  you have to use in order to compile the CSS from the design system. Read the
...
To being able to use the components and compile the CSS from the design system there are a couple of steps to follow before doing everything else.

Assuming you have already installed [PostCSS](https://postcss.org/) on your current stack, the first thing to do is passing Wanda's configuration to it, you can do so by importing `postcssConfig` from `@wonderflow/config` inside your local **postcss.config.js** file.
...
<Snackbar kind="warning" title="Include at least one theme">
  ...
  work as expected. <a href="/develop/themes/">See more</a>.
</Snackbar>
